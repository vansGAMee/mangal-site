    ...
    <AnimatePresence>
      {open ? (
        <motion.div className="fixed inset-0 z-[120] bg-black/65" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} onMouseDown={(event) => { if (event.target === event.currentTarget) onClose(); }}>
          <motion.aside ref={panel} role="dialog" aria-modal="true" aria-labelledby="cart-title" initial={{ x: "100%" }} animate={{ x: 0 }} exit={{ x: "100%" }} transition={{ type: "spring", stiffness: 330, damping: 34 }} className="ml-auto flex h-full w-full max-w-lg flex-col border-l border-white/10 bg-[#121213] p-5 sm:p-8">
            <div className="flex items-center justify-between"><h2 id="cart-title" className="display text-4xl">Корзина</h2><button onClick={onClose} className="grid h-11 w-11 place-items-center border border-white/15 text-2xl" aria-label="Закрыть корзину">×</button></div>
            <div className="mt-7 flex-1 overflow-y-auto" aria-live="polite">
              ...
              {items.map((line) => {
                ...
                return <article key={`${line.productId}:${line.unit}:${line.modifierOptionIds.join(",")}`} className="border-t border-white/10 py-5 first:border-t-0">
                  ...
                  <div className="mt-4 flex items-center gap-1"><button aria-label="Уменьшить количество" className="h-10 w-10 border border-white/10" onClick={() => setQuantity(line, line.quantity - 1)}>−</button><output className="mono grid h-10 min-w-12 place-items-center">{line.quantity}{line.unit === "KILOGRAM" ? " кг" : ""}</output><button aria-label="Увеличить количество" className="h-10 w-10 border border-white/10" onClick={() => setQuantity(line, line.quantity + 1)}>+</button></div>
                </article>;
              })}
            </div>
            <div className="border-t border-white/10 pt-5"><div className="flex justify-between"><span>Предварительная сумма</span><strong className="mono">{formatRubles(total)}</strong></div><p className="mt-2 text-xs leading-5 text-[var(--muted)]">Финальная цена и доступность повторно проверяются сервером при оформлении.</p>{items.length && catalog.store.deliveryZones.length ? <Link href="/checkout" onClick={onClose} className="mt-5 grid min-h-12 w-full place-items-center rounded-[4px] bg-[var(--ember)] px-5 font-semibold text-black">Перейти к оформлению</Link> : <span className="mt-5 grid min-h-12 w-full place-items-center rounded-[4px] bg-[var(--ember)] px-5 font-semibold text-black opacity-45">Перейти к оформлению</span>}{!catalog.store.deliveryZones.length ? <p className="mt-3 text-xs text-[var(--copper)]">Зоны доставки ещё не подтверждены оператором.</p> : null}</div>
          </motion.aside>
        </motion.div>
      ) : null}
    </AnimatePresence>
