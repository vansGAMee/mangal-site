    ...
    <AnimatePresence>
      {product ? (
        <motion.div className="fixed inset-0 z-[130] grid place-items-end bg-black/70 p-0 md:place-items-center md:p-6" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} onMouseDown={(event) => { if (event.target === event.currentTarget) onClose(); }}>
          <motion.section role="dialog" aria-modal="true" aria-labelledby="modifier-title" initial={{ y: 80, opacity: 0 }} animate={{ y: 0, opacity: 1 }} exit={{ y: 60, opacity: 0 }} transition={{ type: "spring", stiffness: 380, damping: 34 }} className="max-h-[92dvh] w-full overflow-y-auto border border-white/10 bg-[#151516] p-5 md:max-w-xl md:rounded-[5px] md:p-8">
            <div className="flex items-start justify-between gap-6">
              ...
              <button ref={closeButton} onClick={onClose} aria-label="Закрыть" className="grid h-11 w-11 place-items-center border border-white/15 text-2xl">×</button>
            </div>
                ...
                <fieldset key={group.id}>
                  ...
                  <div className="grid gap-2">
                    {group.options.map((option) => (
                      <label key={option.id} className="flex min-h-12 items-center justify-between border border-white/10 px-4 py-3 has-[:checked]:border-[var(--ember)]">
                        <span className="flex items-center gap-3"><input type={group.selectionMode === "SINGLE" ? "radio" : "checkbox"} name={group.id} checked={selected.has(option.id)} onChange={() => toggle(group.id, option.id, group.selectionMode === "SINGLE")} disabled={!option.isAvailable} />{option.name}</span>
